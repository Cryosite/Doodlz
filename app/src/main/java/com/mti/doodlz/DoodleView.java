...
public class DoodleView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        }//endif - (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN)
        else
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP) {
            touchEnded(event.getPointerId(actionIndex));
        }//endif - (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP)
        else {
            touchMoved(event);
        }//endelse - (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP)
    ...
    private void touchStarted(float x, float y, int lineID) {
        ...
        }//endif - (pathMap.containsKey(lineID))
        else {
            path = new Path(); //create a new Path
            pathMap.put(lineID, path); //add the Path to Map
            point = new Point(); //create a new Point
            previousPointMap.put(lineID, point); //add the Point to the Map
        }//endelse - (pathMap.containsKey(lineID))

        //move to the coordinates of the touch
        path.moveTo(x, y);
        point.x = (int) x;
        point.y = (int) y;
    }//touchStarted(float x, float y, int lineID)
        ...
        for (int i = 0; i < event.getPointerCount(); i++) {
            ...
            if (pathMap.containsKey(pointerID)) {
                ...
                if (deltaX >= TOUCH_TOLERANCE || deltaY >= TOUCH_TOLERANCE) {
                    ...
                    point.x = (int) newX;
                    point.y = (int) newY;
                }//endif - (deltaX >= TOUCH_TOLERANCE || deltaY >= TOUCH_TOLERANCE)
            }//endif - (pathMap.containsKey(pointerID))
        }//endfor - (int i = 0; i < event.getPointerCount(); i++)
